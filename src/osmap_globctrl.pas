...
Uses
  ...
  mmain, dm, msettings, osmap_datastreams, osmap_geometry, osmap_wkt, Dialogs,
  Variants, LResources;
...
Begin
  ...
  Location.Lat := frmdm.Q.FieldByName('M_LAT').AsFloat;
  Location.Lon := frmdm.Q.FieldByName('M_LON').AsFloat;
  Center := Location;
...
Begin
 if SCount>0 then begin
   Marker.Lat := frmdm.Q.FieldByName('M_LAT').AsFloat;
   Marker.Lon := frmdm.Q.FieldByName('M_LON').AsFloat;
 end;
Refresh;
end;
    ...
    if cursor=crHandPoint then begin
      for i:=0 to High(MapDataset) do begin
        if (X>=MapDataset[i].x-Pointer_Radius) and (X<=MapDataset[i].x+Pointer_Radius) and
           ...
           if frmdm.Q.Locate('ABSNUM', MapDataset[i].ID, [])=true then begin
 ...
 //      showmessage('9.2');
  ...
  //     showmessage('9.3');
     ...
     pen.Style := psSolid;
      ...
      if SCount>0 then begin
       cur_id:=frmdm.Q.FieldByName('ABSNUM').AsInteger;
